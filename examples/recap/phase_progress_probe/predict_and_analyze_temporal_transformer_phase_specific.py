...
"""Predict z/p with the temporal transformer head and analyze value correction."""
...
from train_head_temporal_transformer_phase_specific import TemporalWindowDataset
...
def _load_head(
    ...
    """Load a trained temporal transformer phase-specific head checkpoint."""
    ckpt = torch.load(head_path, map_location="cpu", weights_only=False)
    config = ckpt["config"]
    head = TemporalTransformerPhaseSpecificProgressHead(**config)
...
def _predict_all(
    ...
    """Run the temporal transformer head over cached feature windows."""
    import numpy as np
    ...
    for feature_window, _, _, _ in loader:
        out = head(feature_window.to(device))
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_checkpoint",
        default="/workspace/results/phase_progress_probe/head_temporal_transformer_phase_specific/head.pt",
    )
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/analysis_temporal_transformer_phase_specific",
    )
    ...
    logger.info("Loading temporal transformer head from %s", args.head_checkpoint)
