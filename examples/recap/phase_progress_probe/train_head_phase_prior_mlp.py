...
"""Train a single-frame phase-prior MLP z/p head on cached frozen features."""
...
from model_phase_prior_mlp import PhasePriorMLPHead
...
def _compute_metrics(
    ...
    """Compute validation metrics for the single-frame phase-prior head."""
...
def _build_stage_prior(
    ...
    """Build scheduled stage priors for progress conditioning."""
    gt_onehot = F.one_hot(phase_true.long(), num_classes=args.num_phases).float()

    if epoch <= args.stage_only_epochs + args.gt_stage_prior_epochs or phase_logits is None:
        return gt_onehot

    pred_probs = F.softmax(phase_logits.detach(), dim=-1)
    mix_progress = min(
        1.0,
        max(
            0.0,
...
def evaluate(
    ...
    """Evaluate the single-frame phase-prior head on one split."""
    head.eval()
    total_loss = 0.0
    total_samples = 0
    ...
    for features, phase, phase_progress, global_progress in data_loader:
        ...
        gt_stage_prior = F.one_hot(phase, num_classes=args.num_phases).to(features.dtype)
        out = head(features, stage_prior=gt_stage_prior)
        loss = (
...
def train(
    ...
    """Train the single-frame phase-prior head and return best validation info."""
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for features, phase, phase_progress, global_progress in pbar:
            ...
            if epoch <= args.stage_only_epochs:
                out = head(features, stage_prior=None)
                loss = args.phase_loss_weight * phase_criterion(out["phase_logits"], phase)
            else:
                stage_out = head(features, stage_prior=None)
                stage_prior = _build_stage_prior(
                    phase_true=phase,
                    phase_logits=stage_out["phase_logits"],
                    epoch=epoch,
                    args=args,
                ).to(features.dtype)
                out = head(features, stage_prior=stage_prior)
                loss = (
                    args.phase_loss_weight * phase_criterion(out["phase_logits"], phase)
                    + args.progress_loss_weight
                    * progress_criterion(out["phase_progress"], phase_progress)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/head_phase_prior_mlp",
    )
    parser.add_argument("--num_phases", type=int, default=5)
    parser.add_argument("--hidden_dim", type=int, default=512)
    parser.add_argument("--dropout", type=float, default=0.1)
    parser.add_argument("--trunk_depth", type=int, default=5)
    parser.add_argument("--progress_depth", type=int, default=4)
    parser.add_argument("--stage_embedding_dim", type=int, default=64)
