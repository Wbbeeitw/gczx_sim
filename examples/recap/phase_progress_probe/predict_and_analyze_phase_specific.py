...
"""Predict z/p with the phase-specific head and analyze value correction."""
...
def _load_head(head_path: str, device: torch.device) -> PhaseSpecificProgressHead:
    """Load a trained phase-specific head checkpoint."""
    ckpt = torch.load(head_path, map_location="cpu", weights_only=False)
    config = ckpt["config"]
    head = PhaseSpecificProgressHead(**config)
...
def _predict_all(
    ...
    """Run the phase-specific head over cached features."""
    import numpy as np
...
def _evaluate_correction(
    ...
    """Evaluate raw, oracle, and predicted z/p corrections."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_checkpoint",
        default="/workspace/results/phase_progress_probe/head_phase_specific/head.pt",
    )
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/analysis_phase_specific",
    )
