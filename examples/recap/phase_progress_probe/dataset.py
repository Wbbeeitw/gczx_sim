...
"""Dataset for training a lightweight z/p predictor on frozen VLM features."""
...
def _load_phase_labels(dataset_path: Path, label_name: str = "phase_progress_semantic") -> dict[tuple[int, int], dict[str, Any]]:
    """Load semantic phase labels from ``meta/{label_name}.parquet``.

    Returns a dict keyed by ``(episode_index, frame_index)``.
    """
    ...
    records = df[["episode_index", "frame_index", "phase", "phase_progress", "global_progress"]].to_dict(
        orient="records"
    )
    return {
...
class PhaseProbeDataset(Dataset):
    ...
    attaches pre-computed semantic phase labels.

    Train/val split is performed at the episode level to avoid leakage.
    """
    ...
    def __init__(
        ...
        # Load metadata and build episode-level train/val split.
        ...
        # Build frame indices restricted to selected episodes.
        ...
        # Load task descriptions and phase labels.
        ...
        # Build OpenPI transform pipeline matching the value model.
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        # Inject task description as prompt if available.
