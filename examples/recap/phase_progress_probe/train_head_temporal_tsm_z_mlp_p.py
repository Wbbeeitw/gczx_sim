...
"""Train a temporal TSM + MLP z->p head on cached frozen features."""
...
def evaluate(
    ...
    """Evaluate the temporal TSM head on one split."""
    head.eval()
    total_loss = 0.0
    total_samples = 0
...
def train(
    ...
    """Train the temporal TSM head and return best validation info."""
            ...
            if epoch <= args.stage_only_epochs:
                ...
                stage_prior = _build_stage_prior(
                    ...
                    valid_mask=torch.ones_like(
                        phase_center, dtype=feature_window.dtype
                    ).unsqueeze(1),
                    epoch=epoch,
                    args=args,
                ).squeeze(1)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/head_temporal_tsm_z_mlp_p",
    )
    ...
    torch.save(
        {
            ...
            "head_type": "temporal_tsm_z_mlp_p",
        },
        output_dir / "head.pt",
    )
