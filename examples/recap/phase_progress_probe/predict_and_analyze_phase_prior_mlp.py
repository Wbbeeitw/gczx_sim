...
"""Predict z/p with the single-frame phase-prior MLP and analyze correction."""
...
def _load_head(head_path: str, device: torch.device) -> PhasePriorMLPHead:
    """Load a trained single-frame phase-prior MLP head checkpoint."""
    ckpt = torch.load(head_path, map_location="cpu", weights_only=False)
    config = ckpt["config"]
    head = PhasePriorMLPHead(**config)
...
def _predict_all(
    ...
    """Run the single-frame phase-prior head over cached features."""
    import numpy as np
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_checkpoint",
        default="/workspace/results/phase_progress_probe/head_phase_prior_mlp/head.pt",
    )
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/analysis_phase_prior_mlp",
    )
