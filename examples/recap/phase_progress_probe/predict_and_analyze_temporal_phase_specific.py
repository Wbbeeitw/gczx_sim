...
"""Predict z/p with the temporal phase-specific head and analyze value correction."""
...
from train_head_temporal_phase_specific import TemporalWindowDataset
...
def _load_head(
    head_path: str,
    device: torch.device,
) -> TemporalPhaseSpecificProgressHead:
    """Load a trained temporal phase-specific head checkpoint."""
    ckpt = torch.load(head_path, map_location="cpu", weights_only=False)
    config = ckpt["config"]
    head = TemporalPhaseSpecificProgressHead(**config)
...
def _predict_all(
    ...
    """Run the temporal head over cached feature windows."""
    import numpy as np
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_checkpoint",
        default="/workspace/results/phase_progress_probe/head_temporal_phase_specific/head.pt",
    )
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/analysis_temporal_phase_specific",
    )
    ...
    logger.info("Loading temporal head from %s", args.head_checkpoint)
