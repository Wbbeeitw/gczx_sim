...
"""Predict z/p with the temporal z->MLP->p head and analyze value correction."""
...
def _load_head(
    ...
    """Load a trained temporal z->MLP->p head checkpoint."""
    ckpt = torch.load(head_path, map_location="cpu", weights_only=False)
    config = ckpt["config"]
    head = TemporalZMLPProgressHead(**config)
...
def _predict_all(
    ...
    """Run the temporal z->MLP->p head over cached feature windows."""
    import numpy as np
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_checkpoint",
        default="/workspace/results/phase_progress_probe/head_temporal_z_mlp_p/head.pt",
    )
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/analysis_temporal_z_mlp_p",
    )
    ...
    logger.info("Loading temporal z->MLP->p head from %s", args.head_checkpoint)
