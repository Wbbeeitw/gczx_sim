...
"""Pre-extract frozen VLM prefix features for the phase/progress probe."""
...
from model import VLMBackboneFeatureExtractor
...
def extract_split(
    ...
    """Run the frozen VLM over one split and cache features + labels."""
    feature_extractor.value_model.eval()
    ...
    torch.save({"features": features, **labels}, output_path)
    logger.info(
...
def main() -> None:
    ...
    extract_split(feature_extractor, train_loader, output_dir / "train.pt")
    extract_split(feature_extractor, val_loader, output_dir / "val.pt")
