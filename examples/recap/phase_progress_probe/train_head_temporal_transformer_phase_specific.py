...
"""Train a temporal transformer phase-specific z/p head on cached features."""
...
from model_temporal_transformer_phase_specific import (
    TemporalTransformerPhaseSpecificProgressHead,
)
...
class TemporalWindowDataset(Dataset):
    """Episode-aware temporal window dataset over cached feature tensors."""
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        start = max(0, pos - self.radius)
        end = min(len(rows), pos + self.radius + 1)
        selected = rows[start:end]

        window = self.features.index_select(0, selected)
        if len(selected) < self.window_size:
            left_pad = max(0, self.radius - pos)
            right_pad = max(0, pos + self.radius + 1 - len(rows))
            pad_tensors = []
            if left_pad > 0:
                pad_tensors.append(window[0:1].repeat(left_pad, 1))
            pad_tensors.append(window)
            if right_pad > 0:
                pad_tensors.append(window[-1:].repeat(right_pad, 1))
            window = torch.cat(pad_tensors, dim=0)

        return (
            window,
            self.phase[idx],
            self.phase_progress[idx],
            self.global_progress[idx],
        )
...
def _compute_metrics(
    ...
    """Compute validation metrics for the temporal transformer head."""
...
@torch.no_grad()
def evaluate(
    head: TemporalTransformerPhaseSpecificProgressHead,
    ...
    """Evaluate the temporal transformer head on one split."""
    head.eval()
    total_loss = 0.0
    total_samples = 0
    ...
    for feature_window, phase, phase_progress, global_progress in data_loader:
...
def train(
    ...
    """Train the temporal transformer head and return best validation info."""
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for feature_window, phase, phase_progress, global_progress in pbar:
            feature_window = feature_window.to(device)
            phase = phase.to(device)
            phase_progress = phase_progress.to(device)
            global_progress = global_progress.to(device)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/head_temporal_transformer_phase_specific",
    )
    ...
    torch.save(
        {
            ...
            "head_type": "temporal_transformer_phase_specific_progress",
        },
        output_dir / "head.pt",
    )
