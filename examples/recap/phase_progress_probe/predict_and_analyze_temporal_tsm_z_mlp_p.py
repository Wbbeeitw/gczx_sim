...
"""Predict z/p with the temporal TSM head and analyze value correction."""
...
from model_temporal_tsm_z_mlp_p import TemporalTSMZMLPProgressHead
from predict_and_analyze import _apply_correction, _compute_prediction_metrics
...
def _load_head(
    head_path: str,
    device: torch.device,
) -> TemporalTSMZMLPProgressHead:
    """Load a trained temporal TSM head checkpoint."""
    ckpt = torch.load(head_path, map_location="cpu", weights_only=False)
    config = ckpt["config"]
    head = TemporalTSMZMLPProgressHead(**config)
...
@torch.no_grad()
def _predict_all(
    head: TemporalTSMZMLPProgressHead,
    features_dir: Path,
    device: torch.device,
    batch_size: int,
    splits: list[str],
) -> pd.DataFrame:
    """Run the temporal TSM head over cached feature windows."""
    import numpy as np
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_checkpoint",
        default="/workspace/results/phase_progress_probe/head_temporal_tsm_z_mlp_p/head.pt",
    )
    parser.add_argument(
        "--output_dir",
        default="/workspace/results/phase_progress_probe/analysis_temporal_tsm_z_mlp_p",
    )
    ...
    logger.info("Loading temporal TSM head from %s", args.head_checkpoint)
