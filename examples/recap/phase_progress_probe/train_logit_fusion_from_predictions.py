...
"""Train a logit-space fusion MLP from exported z/p prediction parquet."""
...
def _load_payload(
    ...
    """Load and merge z/p predictions with raw critic logits."""
...
def _build_split_dataset(df: pd.DataFrame) -> TensorDataset:
    """Create a TensorDataset for one split."""
    ...
    raw_value = df["value_current"].to_numpy(dtype=np.float32)
    return_norm = df["return_norm"].to_numpy(dtype=np.float32)
...
def _fused_value_from_logits(
    ...
    """Convert fused logits into scalar value expectation."""
...
def evaluate(
    ...
    """Evaluate fusion MLP on one split."""
    model.eval()
    total_loss = 0.0
    total_samples = 0
    ...
    for raw_logits, phase_probs, phase_progress, global_progress, raw_value, return_norm in data_loader:
        delta = model(
...
def train(
    ...
    """Train a logit-space fusion MLP."""
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for raw_logits, phase_probs, phase_progress, global_progress, _, return_norm in pbar:
            optimizer.zero_grad()
            delta = model(
                raw_logits.to(device),
                phase_probs.to(device),
