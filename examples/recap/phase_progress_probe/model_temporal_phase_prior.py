...
class TemporalPhasePriorProgressHead(nn.Module):
    """Stage-conditioned temporal head with phase-prior global progress.

    This branch keeps the frozen-feature setup but makes two structural changes:
    1. progress prediction is explicitly conditioned on stage priors
    2. global progress uses empirical phase spans instead of equal-length phases
    """
    ...
    def forward(
        ...
        """Predict phase/progress for the entire window and center token.
        ...
        center_phase_progress_soft = phase_progress_all_tokens[:, self.center_index]
        center_phase_progress_hard = center_phase_progress_soft
        global_progress_soft, global_progress_hard = self._compute_global_progress(
            center_phase_probs,
            center_phase_pred,
            center_phase_progress_soft,
            center_phase_progress_hard,
        )
        ...
        return {
            ...
            "phase_progress": center_phase_progress_soft,
            "phase_progress_soft": center_phase_progress_soft,
            "phase_progress_hard": center_phase_progress_hard,
            "global_progress": global_progress_soft,
            "global_progress_soft": global_progress_soft,
            "global_progress_hard": global_progress_hard,
            "phase_logits_all": phase_logits_all,
            "phase_probs_all": phase_probs_all,
            "phase_pred_all": phase_pred_all,
            "phase_progress_all_tokens": phase_progress_all_tokens,
            "stage_prior_used": prior_all,
        }
